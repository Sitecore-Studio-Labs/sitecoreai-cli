...
export const createSetNeverPublishCommand = (): Command => {
  const command = new Command("set-never-publish")
    .description(
      "Set or clear `__Never publish` on a specific item version. Pure CM mutation — does NOT auto-publish. Run `scai publish item` afterwards to push the change to Edge."
    )
