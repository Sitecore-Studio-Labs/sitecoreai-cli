...
layout. Run \`scai workflow inspect "Sample Workflow"\` — if its
...
\`\`\`bash
# Plan first
scai recipe push -i your-workflow.recipe.ts --what-if

# Apply (requires allowWrite on env or --allow-write flag)
scai recipe push -i your-workflow.recipe.ts

# Verify
scai workflow inspect "Your Workflow Name"      # full definition tree
scai workflow list-defs                          # confirm it's there
\`\`\`
