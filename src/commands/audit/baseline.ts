...
export const createAuditBaselineCommand = (): Command => {
  ...
  command.addHelpText(
    ...
    "\nThe baseline lives at `.scai/audit-baseline-<envName>.json` (per-env).\n" +
      ...
      "\nWorkflow:\n" +
      "  1. scai hygiene audit all                    # see all findings\n" +
      "  2. scai hygiene audit baseline create        # accept everything as the new baseline\n" +
      "  3. scai hygiene audit all --baseline         # in CI: only flag NEW findings\n" +
      "\nIncremental accept:\n" +
      "  scai hygiene audit broken-links list --json \\\n" +
      "    | scai hygiene audit baseline accept --audit broken-links\n"
  );
