# `scai hygiene audit` and `scai hygiene cleanup` — scope & known gotchas
...
- After `scai hygiene cleanup` mutates master, the published edge does **not**
  update until a publish runs. Use `scai content publish` (or the Sites API
...
1. **Wait and retry.** Most caches settle within a minute or two.
   `scai hygiene cleanup dead-templates purge --what-if` is a cheap way to
   re-check.
