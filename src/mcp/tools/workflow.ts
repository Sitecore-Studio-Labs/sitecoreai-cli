  ...
  registry.registerTool({
    ...
    inputSchema: {
      verb: z
        ...
        .describe(
          "Which mutation to run. `advance` moves one item through one command. `reset` force-writes one item back to its workflow's initial state (bypasses validation + submit actions). `bulk-advance` sweeps stale items under a root and advances each via a named command (wraps `scai hygiene cleanup workflow advance`). `apply-workflow` attaches a workflow to an item not yet under one (sets `__Workflow` + `__Workflow state` directly). `bulk-apply` does the same as `apply-workflow` but sweeps every item under a root, optionally filtered by template — wraps `scai hygiene cleanup workflow apply`."
        ),
    ...
    handler: async (input, context) => {
      ...
      // via `environmentName`). The per-env `denyMcpElevation` opt-out
      // must be checked against the TARGET env, not the bound one.
