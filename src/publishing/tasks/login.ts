 ...
 * `xmcpub.jobs.t:*` (tenant-tier) scopes live on the standard
 * Sitecore Cloud API audience `https://api.sitecorecloud.io` — the
 * same one scai's deploy operations already target. (The api-docs
 * page also lists `.a` admin-tier variants on a different resource
 * server; those are for Pages-UI user tokens, not automation
 * clients.) Override via SITECOREAI_PUBLISHING_AUDIENCE.
 */
...
/**
 * Interactive device-code login that asks Auth0 for the tenant-tier
 * publishing scopes (`xmcpub.jobs.t:r/w`, `xmcpub.queue:r`). The
 * resulting access token is stored in a publishing-specific keychain
 * key so it doesn't collide with the deploy token (which carries
 * `xmclouddeploy.*` scopes only).
 *
 * For non-interactive / CI use, the env profile's automation client
 * credentials (or `SITECOREAI_ENV_<NAME>_CLIENT_ID` / `_SECRET` env
 * vars) are sufficient — `acquirePublishingToken` mints a fresh M2M
 * token transparently and this command isn't needed.
 *
...
export const runPublishingLogin = async (
  ...
  logger.info(
    `Requesting publishing scopes (xmcpub.jobs.t:r, xmcpub.jobs.t:w, xmcpub.queue:r) from Auth0.`,
    "cyan"
  );
  ...
  const requested = ["xmcpub.jobs.t:r", "xmcpub.jobs.t:w", "xmcpub.queue:r"];
