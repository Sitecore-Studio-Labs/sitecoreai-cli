...
import { runAuditAll, type AuditAllOptions } from "./audit-all";
...
/**
 * `scai audit history <capture|list|diff>` task runners.
 *
...
export const runHistoryList = async (options: HistoryListOptions): Promise<void> => {
  ...
  if (snapshots.length === 0) {
    logger.info("No snapshots yet. Run `scai audit history capture` to take one.", "green");
    return;
  }
...
export const runHistoryDiff = async (options: HistoryDiffOptions): Promise<void> => {
  ...
  if (snapshots.length < 2 && (!options.from || !options.to)) {
    throw createScaiError(
      `Need at least 2 snapshots to diff (have ${snapshots.length}). Run \`scai audit history capture\` first.`,
      "INPUT_INVALID"
    );
  }
