...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  brief_manage:
    "[unstable] Mutating Content Operations Brief surface over a discriminated { resource, verb } input. `resource: 'brief-type'` supports `create` (POST a new type from a full body), `update` (PUT-replace by id; no PATCH — read first if preserving fields), and `delete` (irreversible). `resource: 'brief'` supports `create` (POST a new brief — needs `briefTypeId` plus a `brief` body of name/locale/fields/isTemplate), `update` (partial PUT by id — any subset of name/locale/fields/isTemplate plus an optional `status`), `set-status` — move a brief to Draft | InReview | Approved | Canceled | Archived (a brief must leave Draft before it can be linked to a campaign) — and `delete` (irreversible; SDK `deleteBrief`). `resource: 'comment'` supports `create` — post a comment to a brief via `commentText` (UNVERIFIED: the write body is a best guess; smoke-test before relying on it). Requires allowWrite: true. Brief-type `name` must match /^[A-Za-z][A-Za-z0-9_]*$/. For declarative, idempotent brief writes prefer brief_recipe_push.",
