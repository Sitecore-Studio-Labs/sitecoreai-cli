...
export const createSetValidityCommand = (): Command => {
  const command = new Command("set-validity")
    .description(
      "Set or clear `__Valid from` / `__Valid to` on a specific item version. Pure CM mutation — does NOT auto-publish. Run `scai content publish item` afterwards to push the change to Edge."
    )
