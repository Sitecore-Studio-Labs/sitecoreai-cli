 ...
 *
 * This is the shared auth option shape; it lives in `@/auth` (the
 * cross-domain seam). `serialization/api/types.ts` re-exports it as a
 * published-API forwarder so SDK consumers of
 * `@sitecoreai-labs/sitecoreai-cli/serialization` keep resolving it.
 */
