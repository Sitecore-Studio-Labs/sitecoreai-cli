...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  // Workflow
  workflow_inspect:
    "Read-side workflow surface over a discriminated { verb } input — `get` (auto-routes: a Workflow-templated ref returns the full definition tree (states/commands/actions/validations), any other ref returns one item's workflow + state + available commands), `commands` (transitions available on one item), `definitions` (workflow definitions on the tenant), `status` (per-site rollup from XM Apps REST), or `assigned` (search items by workflow state). The `item` field on verb='get' accepts a GUID, content-tree path, OR a workflow display/item name (case-insensitive). Returns a discriminated `{ kind: 'item' | 'definition', ... }` envelope on the get verb — branch on `result.kind` before reading further. No writes; use this to plan a workflow action before invoking workflow_lifecycle.",
  workflow_lifecycle:
  ...
  brief_inspect:
    "[unstable] Read-side Content Operations Brief surface over a discriminated { verb } input — `list` (briefs in tenant), `get` (one brief by id, with nested to-dos/comments/references), `types` (brief schema templates with field definitions and aiIntent hints), `todos` (across briefs or filtered by briefId), or `comments` (across briefs or filtered by briefId). 'Todo' is the Content Operations UI label for the Brief API's wire `tasks` resource. No writes; safe to call as part of plan assembly before any future brief_manage operation. Sibling: this is the imperative live read surface. For recipe-file-based authoring (capture a brief/brief-type as a declarative recipe, or diff a recipe against live state) use brief_recipe_inspect instead.",
  brief_manage:
    "[unstable] Mutating Content Operations Brief surface over a discriminated { resource, verb } input. `resource: 'brief-type'` supports `create` (POST a new type from a full body), `update` (PUT-replace by id; no PATCH — read first if preserving fields), and `delete` (irreversible). `resource: 'brief'` supports `create` (POST a new brief — needs `briefTypeId` plus a `brief` body of name/locale/fields/isTemplate), `update` (partial PUT by id — any subset of name/locale/fields/isTemplate plus an optional `status` — pass `status: 'Draft'|'InReview'|'Approved'|'Canceled'|'Archived'` to move the brief through its workflow; a brief must leave Draft before it can be linked to a campaign), and `delete` (irreversible; SDK `deleteBrief`). `resource: 'comment'` supports `create` — post a comment to a brief via `commentText` (UNVERIFIED: the write body is a best guess; smoke-test before relying on it). Requires allowWrite: true. Brief-type `name` must match /^[A-Za-z][A-Za-z0-9_]*$/. For declarative, idempotent brief writes prefer brief_recipe_push.",

  // Campaign (Orchestrate)
  campaign_inspect:
    "[unstable] Read-side Orchestrate campaign surface over a discriminated { verb } input — `list` (campaigns in tenant), `get` (one campaign by id, with deliverables and tasks inline), `tasks` (tasks under a deliverable), `task` (one task by id), or `users` (the member directory that resolves the Auth0 subjects on members and assignees). A campaign is an Orchestrate `project`; projects own deliverables, deliverables own tasks. No writes. Sibling: this is the imperative live read surface. For recipe-file-based authoring (capture a campaign as a declarative recipe, or diff a recipe against live state) use campaign_recipe_inspect instead.",
  campaign_manage:
    "[unstable] Mutating Orchestrate campaign surface over a discriminated { resource, verb } input — `resource: 'campaign'|'deliverable'` support verbs `create` and `delete`; `resource: 'task'` supports `create`, `update` (PUT full-replacement, no PATCH), and `delete`. Requires allowWrite: true. Deliverable/task writes need `campaignId` (and `deliverableId` for tasks); update/task-delete need `taskId`. The `delete` verb is irreversible and hits Orchestrate DELETE endpoints that were never captured during reverse-engineering — they are wired optimistically per REST conventions and remain UNVERIFIED; smoke-test before relying on them. Pass whatIf: true for a plan-only dry run. Sibling: this is the imperative write surface (one resource op per call, incl. deletes). For declarative, additive authoring from a recipe file — converge a whole campaign tree at once — use campaign_recipe_push instead.",

  // Agentic Studio
  agents_inspect:
    "[unstable] Read-side Agentic Studio surface over a discriminated { verb } input — `agents`, `skills`, `tools` (the platform tool catalog), `widgets`, `schemas`, `mcps` (registered custom MCP servers), or `status` (browser-session validity and endpoint). Returns { verb, result }. Call this to survey what an environment's Agentic Studio contains before composing a run. No writes. Sibling: this surveys live resources by listing. To capture a single resource (agent/skill/widget/custom-mcp) as a declarative recipe file, or diff a recipe against live state, use agents_recipe_inspect instead; to execute an agent use agents_run.",
  agents_run:
    "[unstable] Runs an Agentic Studio agent by slug against a fresh space and returns the agent's streamed output collected to text. Requires allowWrite: true — a run creates a space and consumes model usage. Find the slug with agents_inspect verb='agents'. Sibling: this EXECUTES an agent. To create or update an agent's definition (not run it), author a recipe and use agents_recipe_push instead.",
  agents_recipe_inspect:
    "[unstable] Pulls or diffs an Agentic Studio resource as a declarative recipe over a { kind, verb } input — `kind` is agent | skill | widget | custom-mcp; `verb='pull'` captures the resource named `name`, `verb='diff'` plans the convergence of `recipe` onto live state. Returns { kind, verb, recipe } or { kind, verb, plan }. No writes. Sibling: this is the recipe-file (declarative) read surface. To browse the live catalog of agents/skills/widgets/mcps imperatively use agents_inspect instead.",
  agents_recipe_push:
    "[unstable] Converges an Agentic Studio resource onto a recipe over a { kind, recipe } input. Requires allowWrite: true; pass whatIf: true for a plan-only dry run. The `agent` kind creates or updates; `skill`/`widget`/`custom-mcp` are create-only (an existing one is left unchanged). Sibling: this is the declarative recipe-file write surface for resource DEFINITIONS. To execute (run) an existing agent rather than define one, use agents_run instead.",

  // Webhook
  webhook_inspect:
    "Read-side webhook handler surface over a discriminated { verb } input — `list` (handler items under /sitecore/system/Webhooks or a workflow state), `get` (one handler's full field detail), or `events` (catalog of strings the tenant accepts for the `events` field on webhook_manage create). Use `events` to discover valid event names BEFORE invoking webhook_manage — typos otherwise surface only at create-time with a generic 'unknown event type' error.",
  webhook_manage:
