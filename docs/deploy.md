...
`scai provision deploy` is a first-class client for the SitecoreAI Deploy API. It
...
Get a token by running `scai setup init` (it can lookup a project/environment
during setup and store the token) or `scai setup login` (refreshes an existing
...
```sh
# CM-only environment
scai provision deploy environments create --project <id> --name <name> --cm-only

# Add an editing host to a CM-only environment
scai provision deploy editing-host create --cm-environment-id <id> --name <name>
```
...
scai provision deploy environments list --project "My Project"
...
scai provision deploy environments health --name <env>

# Upload a deployment from a directory (zips on the fly)
scai provision deploy deployments source --id <deploymentId> --directory ./my-app

# Watch a deployment to completion (bound in CI)
scai provision deploy deployments watch --id <deploymentId> --timeout 3600

# Dry-run any deploy command
scai provision deploy environments create --project "X" --name "Y" --cm-only --what-if
```
