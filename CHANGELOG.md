...
- 5461fd4: Closes the last gap in the ESM build's guard rails.
...
- 202dee8: Surface the underlying cause of a failed Sitecore GraphQL request.
...
- 026deb6: Fixes the broken ESM chunk shipped in 0.40.1, and removes the design that produced it.
...
- 6b6383d: Fixes the dual ESM build breaking browser bundlers.
...
- b1509ab: The package now ships **both CommonJS and ESM**. Every SDK subpath gains an `import` condition resolving to a native ESM build in `dist/esm/`, alongside the existing CommonJS build.
...
- fac707a: Package ownership metadata now carries a team identity rather than an individual's. `package.json` `author` becomes `Sitecore Studio Labs`, with Liz Nelson moved to `contributors` so authorship credit is preserved rather than dropped. The MCPB bundle manifest (`mcpb/manifest.json`) gets the same team `author`.
...
- ee2ab55: Fix phantom `cms-edit` merge conflicts on repeated `recipe push` to the same site — the baseline now records applied state, not desired state.
...
- 8427828: GraphQL transport: detect Cloudflare edge error pages and retry them instead of surfacing an opaque HTML dump under a misleading status
...
- 3d63c92: deploy site bind: preserve an existing site's Start Item instead of requiring `<siteRoot>/Home`
...
- e68a084: Recipe layout compilation now resolves the SXA dynamic-placeholder token `{*}` in nested placeholder keys instead of appending the `DynamicPlaceholderId` after it.
...
- 7d218ed: fix(recipe): converge partial-design scoped datasource slots on template drift
...
- 19c9206: `recipe list --json` now emits `dependsOn` as a true **apply-order** dependency graph and flags it with a new top-level `dependencyModel: "apply-order"` field.
...
- a4fb82b: fix(auth): retry OpenID discovery so a transient network blip doesn't fail the push
...
- edced0e: feat(recipe): strict-by-default push with an opt-in tolerant mode (`SITECOREAI_RECIPE_PUSH_MODE`)
...
- a76db29: fix(recipe): idempotent-by-default ownership guard + ignore inherited `Scai Handle` markers
...
- 016b4eb: fix(recipe): drop scoped datasources on datasource-less renderings instead of aborting the install
...
- 62c7dc5: feat(recipe): `push --from-compiled` honors `--languages` as an apply-time locale filter
...
- ce5f51d: fix(recipe): hoist section-independent shared folders into a stable `__shared-folders__` aggregate
...
- 798dc32: Recipe push: marker-first identity for content-item name-twins. A CreateOnly create whose existing name-twin carries a `Scai Handle` marker from a DIFFERENT recipe family now fails at plan time with a precise cross-recipe name-collision error naming both owners — instead of adopting the foreign item and aborting on the first field its template can't resolve ("Cannot find a field with the name Title/HasPanel", the blank-environment batch aborts). Versionless handle-base compare keeps re-versioned recipes (`@1`→`@2`) owning their items. Additionally, fieldless content-item creates whose fields ride separate SetField ops in the same push are now convergence-eligible (the create carries `retemplateOnAdopt`), closing the eligibility hole that let wrong-template twins through to the field writes.

## 0.34.4

### Patch Changes

- eb3df62: Converge name-twins in batch-separated pushes too — fixes the batch-9 field-op aborts that survived 0.34.3 (`content-item-field:...:en:Title — Cannot find a field with the name Title`, same for new fields like `HasPanel`).
...
- 83450e6: Replace adopt-and-retemplate with real twin convergence — fixes the repeat-install aborts (`retemplating it failed: ... Cannot find a field with the name __Template`).
...
- 488b76a: Fix the v0.34.1 adopt-and-retemplate regression that aborted recipe pushes on repeat installs.
