...
const formatToolsList = (json: boolean, namesOnly: boolean): string => {
  const registry = buildScaiMcpRegistry();
  const tools = registry.listTools();
  if (namesOnly) {
    const names = tools.map((tool) => tool.name);
    return json ? JSON.stringify({ tools: names }, null, 2) : names.join("\n");
  }
  ...
  if (json) {
    return JSON.stringify({ tools: detailed }, null, 2);
  }
  return detailed.map((tool) => `${tool.name}\t[${tool.auth}]\t${tool.description}`).join("\n");
};
