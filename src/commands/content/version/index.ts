...
export const createContentVersionCommand = (): Command => {
  const command = new Command("version").description(
    "Per-version content-state controls. These verbs write CM-side publish-state fields (`__Never publish`, `__Valid from`, `__Valid to`) on a single item version. They do NOT auto-publish — run `scai publish item` separately to push changes to Edge."
  );
