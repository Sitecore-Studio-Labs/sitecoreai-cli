...
// stubbed (no real config/keychain) while the rest of auth stays real.
vi.mock("../../../src/mcp/auth", async () => {
  const actual =
    await vi.importActual<typeof import("../../../src/mcp/auth")>("../../../src/mcp/auth");
  return { ...actual, resolveEnvBinding: gateMocks.resolveEnvBinding };
});
...
describe("dispatchTool — multi-env retargeting gate", () => {
  beforeEach(() => {
    gateMocks.resolveEnvBinding.mockReset();
    gateMocks.ensureMcpElevationAllowed.mockReset();
  });

  const writeDescriptor = (handler: ToolDescriptor["handler"]): ToolDescriptor => ({
    name: "demo_write",
    description: minDescription,
    annotations: baseAnnotations,
    auth: "write",
    inputSchema: { allowWrite: z.boolean(), environmentName: z.string().optional() },
    handler,
  });

  it("checks the TARGET env's denyMcpElevation when a write tool is retargeted", async () => {
    ...
    const result = await dispatchTool(
      writeDescriptor(handler),
      { allowWrite: true, environmentName: "prod" },
      { context: baseContext, extra: makeExtra() }
    );
    expect(result.isError).toBeUndefined();
  ...
  it("rejects the call when the target env denies MCP elevation", async () => {
    gateMocks.resolveEnvBinding.mockResolvedValue({
      envName: "prod",
      resolved: {
        root: { environments: {} },
    ...
    gateMocks.ensureMcpElevationAllowed.mockImplementation(() => {
      throw createScaiError("denied", "AUTH_DENIED");
    });
    const handler = vi.fn();
    const result = await dispatchTool(
      writeDescriptor(handler),
    ...
    expect(result.isError).toBe(true);
    expect((result.structuredContent as { code: string }).code).toBe("AUTH_DENIED");
    expect(handler).not.toHaveBeenCalled();
  });

  it("does NOT resolve a binding when the write targets the bound env (no environmentName)", async () => {
    const handler = vi.fn(async () => ({ content: [{ type: "text" as const, text: "ok" }] }));
    await dispatchTool(
      writeDescriptor(handler),
      { allowWrite: true },
    ...
    expect(gateMocks.resolveEnvBinding).not.toHaveBeenCalled();
    expect(gateMocks.ensureMcpElevationAllowed).not.toHaveBeenCalled();
  });

  it("does NOT resolve a binding when environmentName equals the bound env", async () => {
    const handler = vi.fn(async () => ({ content: [{ type: "text" as const, text: "ok" }] }));
    await dispatchTool(
      writeDescriptor(handler),
      { allowWrite: true, environmentName: "test-env" },
      { context: baseContext, extra: makeExtra() }
    );
    expect(gateMocks.resolveEnvBinding).not.toHaveBeenCalled();
  });
