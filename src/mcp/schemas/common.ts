...
export const environmentBindingShape = {
  environmentName: z
    ...
    .optional()
    .describe(
      "Target environment; defaults to the server's bound environment. When set, the call is resolved against the named environment (config, deploy token, and allowWrite are all derived from it). Use `scai_overview` to list configured environments."
    ),
} as const;
