...
>
> ```bash
> scai content workflow inspect "Sample Workflow"
> ```
>
...
`scai content workflow advance`) executes a workflow command, Sitecore:
...
- Older XM Cloud SKU that pre-dates the webhook-actions templates.
  Verify by running `scai content workflow inspect "Sample Workflow"` — if
...
**`scai provision recipe push --what-if` shows
...
**`scai content workflow inspect "Workflow Name"` returns "No workflow
definition matched"**
...
or GUID. Use `scai content workflow list-defs --json` to enumerate.
...
```bash
scai content workflow inspect "Blog Article Approval"   # full definition tree
scai content workflow list-defs --json                  # confirm it shows up
scai content workflow advance /sitecore/content/MySite/AnArticle \
