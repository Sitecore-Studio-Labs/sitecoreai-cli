...
import { allowWriteShape, paginationShape } from "../schemas/common";
...
export const registerDeployTools = (registry: McpRegistry): void => {
  registry.registerTool({
    ...
    handler: async (_input, context) => {
      const options = apiOptionsFromContext(context.deployToken);
  ...
  registry.registerTool({
    ...
    handler: async (input, context) => {
      ...
      if (input.includeLogs) {
        try {
          logs = await fetchDeploymentLogs(input.deploymentId, context.deployToken);
        } catch {
          logs = null;
        }
      }
    ...
    handler: async (input, context) => {
      ...
      if (input.deploymentId) {
        const result = await deployDeployment(
          options,
          input.deploymentId,
          context.resolved.environment.organizationId
        );
        return {
  ...
  registry.registerTool({
    ...
    handler: async (input, context) => {
      const options = apiOptionsFromContext(context.deployToken);
      const orgId = context.resolved.environment.organizationId;
      switch (input.scope) {
        case "integrations": {
