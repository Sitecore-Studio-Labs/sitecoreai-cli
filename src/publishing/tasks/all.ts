...
export const runPublishAll = async (options: RunPublishAllOptions): Promise<void> => {
  ...
  if (whatIf) {
    ...
    // Pre-flight: (1) surface the most recent whole-env publish as a
    // "modified since" baseline for Smart mode; (2) warn when another
    // whole-env publish is already queued or running, since the API
    // serializes these (verified 2026-05-14 — second submission waited
    // ~80s behind the first). Both checks share one token acquisition;
    ...
    try {
      ...
      if (lastPublish) {
        ...
        const ts =
          lastPublish.completedAt ?? lastPublish.raw.system?.finishTime ?? "(time unknown)";
      ...
      const inflight = await listPublishJobs(client, {
