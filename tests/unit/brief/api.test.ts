...
describe("brief instance CRUD", () => {
  ...
  it("linkBriefToProject PATCHes /links with the AI/project body and returns undefined on 204", async () => {
    const fetchMock = vi.fn().mockResolvedValue(okResponse(null, 204));
    vi.stubGlobal("fetch", fetchMock);

    const result = await linkBriefToProject(baseOptions, "brief-xyz", "proj-123");
  ...
  it("linkBriefToProject URL-encodes the brief id", async () => {
    const fetchMock = vi.fn().mockResolvedValue(okResponse(null, 204));
    vi.stubGlobal("fetch", fetchMock);
