  ...
  registry.registerTool({
    ...
    handler: async (input, context) => {
      ...
      const compileCtx = resolveCompileContext(
        context.resolved.environment as unknown as Record<string, unknown>,
        input.roots
      );
  ...
  registry.registerTool({
    ...
    handler: async (input, context) => {
      const results = await runRecipeDiff(
        baseTaskOptions(context.configPath, context.envName, { input: input.inputPath }) as never
      );
      return {
        content: [
          {
            type: "text",
            text: `Diffed ${results.length} recipe action(s) against '${context.envName}'.`,
          },
        ],
    ...
    handler: async (input, context) => {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Plan for '${plan.recipeHandle}' on '${context.envName}': ${total} action(s) — create:${plan.summary.create}, update:${plan.summary.update}, skip:${plan.summary.skip}, error:${plan.summary.error}.`,
          },
        ],
        structuredContent: {
    ...
    handler: async (input, context, extra) => {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Pushed ${results.length} recipe(s) to '${context.envName}'${input.whatIf ? " (whatIf)" : ""}: ${succeeded} succeeded, ${failed} aborted.`,
          },
        ],
        structuredContent: {
          environment: context.envName,
