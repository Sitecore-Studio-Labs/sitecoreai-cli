 ...
 *
 * Verbs map 1:1 with the `scai hygiene cleanup` CLI verbs whose library task
...
const buildRunnerOptions = (
  ...
  for (const [k, v] of Object.entries(input)) {
    ...
    if (k === "verb" || k === "whatIf" || k === "allowWrite" || k === "environmentName") continue;
...
export const registerCleanupTools = (registry: McpRegistry): void => {
  ...
  registry.registerTool({
    ...
    handler: async (input, context) => {
      ...
      // Per-env opt-out for MCP-elevated writes, checked against the
      // TARGET env. Only enforced when this call would actually mutate
      // the tenant — what-if previews are read-only and always permitted.
      if (!whatIf) {
      ...
      const opts = buildRunnerOptions(
        input.verb,
        input as Record<string, unknown>,
        baseTaskOptions(context.configPath, binding.envName, {
          allowWrite: !whatIf,
        }),
        whatIf
      );
...
const cleanupInputSchema = () =>
  ({
    ...
    checkRefs: z
      ...
      .optional()
      .describe(
        "slug-conflicts: pre-scan inbound references per loser. Preview mode attaches counts to each resolved row (warn-only). Apply mode (`whatIf: false`) aborts the call when any loser has positive counts — INPUT_INVALID with the blocker list. Off by default; cost scales with number of losers."
      ),
    refCheckRoot: z
      .string()
      .optional()
      .describe(
        "slug-conflicts: content root for the inbound-ref scan when `checkRefs: true`. Default `/sitecore` (full tenant). Narrow on large tenants if you know refs only come from a subtree."
      ),
