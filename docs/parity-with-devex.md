...
| ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------ | ------------------------------------------------------------ |
| `sitecore ser pull` | `scai provision ser pull` | Same semantics. |
| `sitecore ser push` | `scai provision ser push` | Same semantics. The `--publish` chain flag is intentionally not ported — publish is a separate consent-gated verb (see Publishing below); chaining would bypass the safety model. |
| `sitecore ser diff` (local vs remote) | `scai provision ser diff` | Same semantics for the local-vs-remote case. |
| `sitecore ser diff --source A --destination B [--push]` | `scai provision ser diff --source-env A --target-env B [--push]` | Same semantics. Source/destination metadata fetched in parallel; per-item fetch fanout bounded by `SITECOREAI_HTTP_CONCURRENCY`. Adds `--what-if`, `--allow-write`, `--force` (empty-source guard). Shipped 2026-05-14. |
| `sitecore ser info`, `explain`, `validate`, `watch` | `scai provision ser info                                                | explain                                                                                                                                                                                                                 | validate                                               | watch` | Same. `--fix` auto-correct on `validate` is not implemented. |
| `sitecore ser package create                                                         | install` | `scai provision ser package create                                                                                                                                                                                      | install` | Same. |
| `sitecore cloud organization {info,health,license}` | `scai provision deploy org {get,health,license,launch-demo}` | scai adds `launch-demo`. |
| `sitecore cloud project {create,list,info,update,delete}` | `scai provision deploy proj {…}` | scai adds `limitation`, `validate-name`, `link-repository`, `unlink-repository`. |
| `sitecore cloud environment {create,list,info,update,delete,health,restart,promote}` | `scai provision deploy env {…}` including `health` | scai adds `get-edge-token`, `get-editing-secret`, `regenerate-context`, repo linking. `env health` probes `<cmHost>/healthz/ready`. |
| `sitecore cloud environment variable {list,upsert,delete}` | `scai provision deploy env variables {…}` | Same. |
| `sitecore cloud deployment {create,list,info,start,watch,cancel}` | `scai provision deploy dep {…}` | Same. |
| `sitecore cloud editinghost {create,update,delete,deploy}` | `scai provision deploy editing-host {list,create,update,delete,deploy}` | scai adds `list`. |
| `sitecore cloud logs {list,view,download}` + `deployment log` | `scai provision deploy logs {list,view,data}` | Same. |
| `sitecore cloud login                                                                | logout` | `scai setup login` / `scai setup logout` | Top-level on scai; OS keychain instead of `user.json`. |
| `sitecore init` (sitecore.json) | `scai setup init` | Different config file (`sitecoreai.cli.json`) and an interactive wizard. |

## Added in scai

Features that have no dotnet counterpart:

- **Recipes** (`scai provision recipe {compile,plan,diff,push,prune-defaults}`) —
...
- **Local activity log** — `scai cli history` records redacted commands at
  `~/.sitecoreai/cli-history.log`.
- **Interactive REPL** — `scai cli shell`.
- **`scai provision deploy site`** and **`scai provision deploy source-control`** command
  groups — first-class Deploy API surfaces that the dotnet plugin
  didn't expose.
- **Telemetry honoring `DO_NOT_TRACK`** — opt-out via the standard
  Console Do Not Track env var, or `SITECOREAI_TELEMETRY=false`.
...
- `scai content publish item --path <id-or-path> [--languages …]
[--include-subitems] [--include-related]` → item / subtree publish.
- `scai content publish all [--languages …]` → whole-environment republish to
  Edge. Real ops button (needed sometimes after big serialization
  pushes, rollbacks, or migrations where Edge has drifted from CM).
...
- `scai content publish cancel <jobId>` → `POST /jobs/{jobId}/cancel`.
  First-class because `publish all` is a heavy operation that
  operators may need to abort mid-flight.

Bare `scai content publish` (no subcommand) errors out — no accidental
invocation path.
...
#### Tier 1 — item / subtree publish (`scai content publish item`)
...
1. **CLI defaults to `--what-if`.** Running without `--allow-write`
   ...
   as `scai hygiene cleanup versions prune`.
...
#### Tier 2 — whole-environment republish (`scai content publish all`)
...
- `scai hygiene audit` (read-only diagnostics) — broken-links, unused-media,
  orphans (= XM Cloud archive listing), stale-workflow, language-data.
- `scai hygiene cleanup` (mutating) — versions prune with `--root` /
...
- `clean-invalid-language-data` analogue is **read-only** (`scai hygiene audit
