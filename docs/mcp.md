...
# 2. Authenticate.
scai login --environment-name dev
...
stdio only in v1. The server reads JSON-RPC frames from stdin and
writes them to stdout, one frame per line.

HTTP / SSE transports are deferred to v2. The library plumbing
already supports them via the SDK's `StreamableHTTPServerTransport`
...
```json
{
  ...
  "structuredContent": {
    ...
    "why": "Token missing.",
    "hint": "Run scai login.",
    "next": "Run `scai login` for the bound environment, then restart the MCP server.",
