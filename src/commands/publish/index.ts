...
export const createPublishCommand = (): Command => {
  ...
  command.addHelpText(
    "after",
    "\nExamples:\n" +
      "  $ scai publish item --item-id <guid> -n <env>                # dry-run, prints scope + token\n" +
      "  $ scai publish item --item-id <guid> -n <env> --allow-write  # actually publish (non-prod)\n" +
      "  $ scai publish status <jobId> -n <env>                       # inspect a publish job\n" +
      "\nSetup:\n" +
