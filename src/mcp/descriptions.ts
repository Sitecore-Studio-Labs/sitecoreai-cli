...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  brief_inspect:
    "Read-side Content Operations Brief surface over a discriminated { verb } input — `list` (briefs in tenant), `show` (one brief by id, with nested tasks/comments/references), `types` (brief schema templates with field definitions and aiIntent hints), `tasks` (across briefs or filtered by briefId), or `comments` (across briefs or filtered by briefId). No writes; safe to call as part of plan assembly before any future brief_manage operation.",
  brief_manage:
    "Mutating Content Operations Brief surface over a discriminated { resource, verb } input. `resource: 'brief-type'` supports `create` (POST a new type from a full body), `update` (PUT-replace by id; no PATCH — read first if preserving fields), and `delete` (irreversible). `resource: 'brief'` supports `set-status` — move a brief to Draft | InReview | Approved | Canceled | Archived (a brief must leave Draft before it can be linked to a campaign). Requires allowWrite: true. Brief-type `name` must match /^[A-Za-z][A-Za-z0-9_]*$/. Brief create/delete remain SDK/CLI-only.",

  // Campaign (Orchestrate)
  campaign_inspect:
    "Read-side Orchestrate campaign surface over a discriminated { verb } input — `list` (campaigns in tenant), `show` (one campaign by id, with deliverables and tasks inline), `tasks` (tasks under a deliverable), `task` (one task by id), or `users` (the member directory that resolves the Auth0 subjects on members and assignees). A campaign is an Orchestrate `project`; projects own deliverables, deliverables own tasks. No writes.",
  campaign_manage:
    "Mutating Orchestrate campaign surface over a discriminated { resource, verb } input — `resource: 'campaign'|'deliverable'` support verb `create`; `resource: 'task'` supports `create` and `update` (PUT full-replacement, no PATCH). Requires allowWrite: true. Deliverable/task writes need `campaignId` (and `deliverableId` for tasks); update needs `taskId`. Pass whatIf: true for a plan-only dry run.",

  // Agentic Studio
  agents_inspect:
    "Read-side Agentic Studio surface over a discriminated { verb } input — `agents`, `skills`, `tools` (the platform tool catalog), `widgets`, `schemas`, `mcps` (registered custom MCP servers), or `status` (browser-session validity and endpoint). Returns { verb, result }. Call this to survey what an environment's Agentic Studio contains before composing a run. No writes.",
  agents_run:
    "Runs an Agentic Studio agent by slug against a fresh space and returns the agent's streamed output collected to text. Requires allowWrite: true — a run creates a space and consumes model usage. Find the slug with agents_inspect verb='agents'.",
  agents_recipe_inspect:
    "Pulls or diffs an Agentic Studio resource as a declarative recipe over a { kind, verb } input — `kind` is agent | skill | widget | custom-mcp; `verb='pull'` captures the resource named `name`, `verb='diff'` plans the convergence of `recipe` onto live state. Returns { kind, verb, recipe } or { kind, verb, plan }. No writes.",
  agents_recipe_push:
    "Converges an Agentic Studio resource onto a recipe over a { kind, recipe } input. Requires allowWrite: true; pass whatIf: true for a plan-only dry run. The `agent` kind creates or updates; `skill`/`widget`/`custom-mcp` are create-only (an existing one is left unchanged).",
